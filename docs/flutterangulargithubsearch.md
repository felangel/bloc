...
?> **Note:** We create a custom `EventTransformer` to [throttle](https://en.wikipedia.org/wiki/Throttling_process_(computing)) the `GithubSearchEvents`. One of the reasons why we created a `Bloc` instead of a `Cubit` was to take advantage of these reactive operators.
