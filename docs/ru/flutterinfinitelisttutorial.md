...
Затем нам нужно реализовать `on<PostFetched>`, который будет запускаться каждый раз, когда добавляется `PostEvent`.
...
?> **Примечание:** Переопределение позволяет нам преобразовать `Stream<Event>` до вызова `on<PostFetched>`. Это позволяет применять такие операции, как Different(), debounceTime() и т.д.
