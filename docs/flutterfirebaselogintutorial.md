...
It's time to implement our `LoginBloc`. As always, we need to extend `Bloc` and define our initial state as well as `mapEventToState`.
...
Just as before, we need to extend `Bloc`, set the initial state, and implement `mapEventToState`. Optionally, we are overriding `transformEvents` again so that we can give users some time to finish typing before we validate the form.
