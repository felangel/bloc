...
?> jsonplaceholder是一个在线的REST API. 它可以生成用于测试的虚假数据, 正好可以用于我们的原型应用开发.
...
?> **附注:** 在url中我们制定了start和limit作为GET请求的查询参数

好的, 现在我们知道数据长的是什么样子了. 让我们建立数据模型吧

## 数据模型

创建 `post.dart`. 我们会在这里进行创建POST对象模型的工作.
...
现在我们有了`Post`对象的模型. 下一步让我们开始写我们的**业务逻辑组件(bloc)**

## Post 事件(Events)

在我们开始实现bloc前, 我们需要定义`PostBloc`之后要做什么.

总体来看, 它会相应用户的输入(滚动屏幕)和请求更多的post数据并且送给表现层显示给用户. 让我们先创建我们的`Event`

我们的`PostBloc`将会只响应单独的事件; 当表现层需要更多Post展示时, 他将会创建和添加`Fetch`事件. 
...
复习一下之前说过的内容, `PostBloc`会接收`PostEvents`, 并且把他们转化成`PostStates`. 
我们已经定义了所有我们需要用到的`PostEvents`(Fetch), 接下来让我们定义我们的`PostState`.

## Post 状态(States)

我们的表现层需要以下的几部分信息:

- `PostUninitialized`(Post 未初始化) - 会告诉表现层它需要渲染加载进度条, 当加载第一批post的时候.

- `PostLoaded`(Post 已加载) - 会告诉表现层它有了可以渲染的内容.
...
?> 我们实现了`copyWith`, 这样我们就可以复制一个`PostLoaded`的实例, 很方便的修改其中的0个到多个属性了(之后用起来很方便).
...
简单起见, `PostBloc`会直接依赖`http client`; 然而, 在生产应用中, 你可能希望注入一个客户端api同时使用repository设计模式[docs](./architecture.md).
...
我们可以开始实现`initialState`了. 在接收到任何事件(Event)之前, `initialState`将会成为`PostBloc`的默认状态(State).

[post_bloc.dart](../_snippets/flutter_infinite_list_tutorial/post_bloc_initial_state.dart.md ':include')

下一步, 我们需要实现`mapEventToState`方法. 每次加入`PostEvent`的时候我们都会调用这个函数.
...
在`App`widget中, 我们使用`BlocProvider`来创建和subtree(指`BlocProvider`下的子widgets)提供`PostBloc`的实例.
而且我们给bloc添加了一个`Fetch`事件, 这样当应用载入的时候, 它就会请求最开始的一批post了.

[main.dart](../_snippets/flutter_infinite_list_tutorial/main.dart.md ':include')

接下来, 我们需要实现我们的`HomePage`widget. `HomePage`会展示加载好的post并与`PostBloc`进行交互.

[home_page.dart](../_snippets/flutter_infinite_list_tutorial/home_page.dart.md ':include')

?> `HomePage`是一个`StatefulWidget`(有状态的widget), 因为它需要维护`ScrollController`. 在`initState`, 我们给`ScrollController`添加了监听器, 这样我们就可以相应滚动事件了. 我们可以通过`BlocProvider.of<PostBloc>(context)`来访问`PostBloc`实例.
...
当有任何用户滚动时, 我们会计算现在距离页面底部还有多远. 如果距离≤`_scrollThreshold`的话, 我们
...
作为使用bloc库的一个加分点, 我们可以在同一个地方操作所有的`Transitions(状态转移)`

> 从一个状态转变成另一个状态的过程叫做`Transition(状态转移)`

?> 一个`Transition(转变)`由 当前状态, 事件, 和下一个状态 组成.

虽然在这个应用中我们只有一个bloc, 但是在真正的大型应用程序中使用多个bloc管理应用的不同部分的`state(状态)`也很常见

如果我们在每次`Transition(状态转移)`时都做一些事情响应的话, 我们只需要创建自己的`BlocDelegate`就可以了.

[simple_bloc_delegate.dart](../_snippets/flutter_infinite_list_tutorial/simple_bloc_delegate.dart.md ':include')

?> 我们**只需要**继承`BlocDelegate`并重写`onTransition`方法.
...
这就是所有我要介绍的事情了! 我们现在已经成功的用[bloc](https://pub.dev/packages/bloc)和[flutter_bloc](https://pub.dev/packages/flutter_bloc)实现了无限列表, 并且这样实现可以将表现层和业务逻辑分离开来.

`HomePage(主页面)`完全不知道`Post`从哪里来, 也完全不知道`Post`的数据是怎么加载到的. 而且`PostBloc`也同样不知道`State(状态)`是怎么被渲染到屏幕的, 他只是很简单的把`Event(事件)`转化成`State(事件)`
