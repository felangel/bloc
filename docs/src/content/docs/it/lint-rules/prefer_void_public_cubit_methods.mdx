...
I metodi pubblici su istanze `Cubit` dovrebbero essere usati per notificare il `Cubit` e
iniziare cambiamenti di stato tramite il metodo `emit`. Se il chiamante ha bisogno di accedere a qualsiasi
informazione di stato, dovrebbero accedervi trammite `state`.

:::note

Le seguenti regole sono correlate e sono solitamente abilitate in combinazione con
