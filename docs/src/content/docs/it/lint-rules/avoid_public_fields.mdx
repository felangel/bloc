...
I componenti di logica applicativa mantengono il proprio `state` ed emettono cambiamenti di stato tramite
l'API `emit`. Di conseguenza, tutto lo stato pubblico dovrebbe essere esposto tramite l'oggetto
`state`.
...
Per abilitare la regola `avoid_public_fields`, aggiungila al tuo `analysis_options.yaml`
sotto `bloc` > `rules`:
