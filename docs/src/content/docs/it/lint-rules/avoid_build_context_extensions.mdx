...
Evita di usare estensioni di `BuildContext` per accedere a istanze di `Bloc` o `Cubit`.
...
metodi di bloc rispetto le estensioni di `BuildContext`. Questo è anche vantaggioso
per il testing dato che non è possibile mockare un metodo di una estensione.
...
**Evita** di usare estensioni `BuildContext` per interagire con istanze di `Bloc` o `Cubit`.
