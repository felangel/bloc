...
:::note Por favor, certifique-se de ler atentamente as seções a seguir antes de
...
:::note Para mais informações sobre as origens do `Cubit`, consulte o seguinte
questão [aqui](https://github.com/felangel/cubit/issues/69). :::
...
:::caution O método `emit` é protegido, o que significa que deve ser usado
apenas dentro de um `Cubit`. :::
...
:::note `await Future.delayed(Duration.zero)` é adicionado a este exemplo para
evitar cancelar a assinatura imediatamente. :::

:::caution Somente as mudanças de estado subsequentes serão recebidas quando
chamamos `listen` em um `Cubit`. :::
...
:::note Uma `Change` ocorre logo antes do estado do `Cubit` ser atualizado. Uma
`Change` consiste em no `currentState` e no `nextState`. :::
...
:::note Tudo que precisamos fazer é extender `BlocObserver` e substituir o
método `onChange`. :::
...
:::note A sobrescrita interna do `onChange` é chamada primeiro, que chama
`super.onChange` notificando o `onChange` no `BlocObserver`. :::

:::tip No `BlocObserver` temos acesso à instância do `Cubit` além da própria
`Change`. :::
...
:::note `onError` pode ser sobrescrito dentro do `Cubit` para manipular todos os
erros de um `Cubit` específico. :::
...
:::tip Um `EventHandler` tem acesso ao evento adicionado, bem como a um
`Emitter`, que pode ser usado para emitir zero ou mais estados em resposta ao
evento recebido. :::
...
:::note Como a classe `Bloc` estende `BlocBase`, temos acesso ao estado atual do
bloc em qualquer momento via o getter `state`, assim como no `Cubit`. :::

:::caution Blocs nunca devem `emitir` novos estados diretamente. Em vez disso,
toda mudança de estado deve ser emitida em resposta a um evento de entrada
dentro de um `EventHandler`. :::

:::caution Ambos blocs e cubits irão ignorar estados duplicados. Se emitirmos
...
:::note `await Future.delayed(Duration.zero)` é adicionado para garantir que
aguardamos a próxima iteração do event-loop (permitindo que o `EventHandler`
processe o evento). :::
...
:::note `await Future.delayed(Duration.zero)` é adicionado neste exemplo para
evitar o cancelamento imediato da assinatura. :::
...
:::note `onTransition` é invocado antes de `onChange` e contém o evento que
acionou a mudança de `currentState` para `nextState`. :::
...
:::note `onTransition` é invocado primeiro (local antes de global), seguido por
`onChange`. :::
...
:::note `onEvent` é chamado assim que o evento é adicionado. O `onEvent` local é
...
:::note O `onError` local é invocado primeiro, seguido pelo `onError` global no
`BlocObserver`. :::

:::note `onError` e `onChange` funcionam exatamente da mesma maneira para
instâncias `Bloc` e `Cubit`. :::

:::caution Quaisquer exceções não tratadas que ocorram em um `EventHandler`
também são reportadas no `onError`. :::
