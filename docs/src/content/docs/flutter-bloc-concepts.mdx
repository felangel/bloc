...
:::note
All widgets exported by the `flutter_bloc` package integrate with both `Cubit` and `Bloc` instances.
:::
...
Since we only have one repository in our app, we will inject it into our widget tree via `RepositoryProvider`.
...
:::note
`context.read<T>()` does not listen to `T` -- if the provided `Object` of type `T` changes, `context.read` will not trigger a widget rebuild.
:::
