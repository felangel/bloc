...
description: Детальний посібник зі створення додатку-таймера на Flutter з використанням bloc.
sidebar:
  order: 2
---
...
- якщо стан `TimerRunInProgress`, користувач зможе призупинити та скинути таймер,
  а також бачити тривалість, що залишилася.
- якщо стан `TimerRunPause`, користувач зможе відновити таймер та скинути таймер.
...
Ми також визначаємо `StreamSubscription` для нашого `Ticker`, до якої повернемося
трохи пізніше.
...
Кожного разу, коли отримується подія `_TimerTicked`, якщо тривалість тіку
більше 0, нам потрібно встановити оновлений стан `TimerRunInProgress` з новою
...
`builder`, можна надати необов'язковий `buildWhen` для `BlocBuilder`. `buildWhen`
приймає попередній стан bloc та поточний стан bloc і повертає `boolean`. Якщо
`buildWhen` повертає `true`, `builder` буде викликаний зі `state`, і віджет
перебудується. Якщо `buildWhen` повертає `false`, `builder` не буде викликаний
зі `state`, і перебудова не відбудеться.
