...
description:
  Детальний посібник зі створення нескінченного списку на Flutter з використанням
  bloc.
sidebar:
  order: 3
---
...
завантажуватиме більше постів, щоб шар представлення міг їх відобразити.
Почнемо зі створення нашої `Event`.

Наш `PostBloc` реагуватиме лише на одну подію — `PostFetched`, яку буде
додавати шар представлення, коли йому потрібно більше постів для відображення.
Оскільки наш `PostFetched` є типом `PostEvent`, ми можемо створити
`bloc/post_event.dart` та реалізувати подію так.
...
Ми реалізували `copyWith`, щоб мати можливість копіювати екземпляр
`PostSuccess` та зручно оновлювати нуль або більше властивостей (це знадобиться
пізніше).
...
Лише з оголошення класу ми бачимо, що наш PostBloc прийматиме PostEvents як
вхід та виводитиме PostStates.
...
обробнику подій. Перегляньте [основні концепції](/uk/bloc-concepts#streams) для
...
Одна оптимізація, яку ми можемо зробити — це `throttle` подій `PostFetched`,
щоб не спамити наш API без потреби. Ми можемо це зробити, використовуючи
параметр `transform` при реєстрації обробника подій `_onFetched`.

:::note

Передача `transformer` до `on<PostFetched>` дозволяє налаштувати спосіб
обробки подій.
...
Чудово! Тепер, коли ми завершили реалізацію бізнес-логіки, залишилося реалізувати
шар представлення.
...
Далі наш метод build повертає `BlocBuilder`. `BlocBuilder` — це віджет Flutter
з пакету [flutter_bloc](https://pub.dev/packages/flutter_bloc), який обробляє
...
Додатковою перевагою використання бібліотеки bloc є те, що ми можемо мати
доступ до всіх `Transitions` в одному місці.
...
Все, що нам потрібно зробити — це розширити `BlocObserver` та перевизначити метод
`onTransition`.
