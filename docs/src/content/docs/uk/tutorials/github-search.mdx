...
У наступному посібнику ми створимо додаток пошуку GitHub у Flutter та AngularDart,
щоб продемонструвати, як ми можемо спільно використовувати шари даних та
бізнес-логіки між двома проєктами.
...
Реалізація `SearchResult` залежить від `SearchResultItem.fromJson`, який ми ще не
реалізували.
...
відповідатиме за
[мемоізацію](https://en.wikipedia.org/wiki/Memoization) як оптимізацію
продуктивності.
...
провайдери даних, сховища та блоки з `common_github_search` для реалізації пошуку
Github.
...
`_SearchBar` також буде `StatefulWidget`, тому що йому потрібно підтримувати свій
власний `TextEditingController`, щоб ми могли відстежувати, що користувач ввів.
...
`_SearchResults` -- це `StatelessWidget`, який приймає `List<SearchResultItem>` і
відображає їх як список `_SearchResultItems`.
...
стану. Оскільки параметр bloc об'єкта `BlocBuilder` було пропущено, `BlocBuilder`
автоматично виконає пошук за допомогою `BlocProvider` та поточного
...
Ми використовуємо пакет [url_launcher](https://pub.dev/packages/url_launcher) для
відкриття зовнішніх URL-адрес.
...
Ось і все! Ми успішно реалізували додаток пошуку GitHub у Flutter, використовуючи
...
`SearchResultItem` -- це компонент, який відповідає за відображення інформації про
один результат пошуку. Він також відповідає за обробку взаємодії користувача та
навігацію за URL-адресою репозиторію при натисканні користувача.
...
поширено, коли веб-додатки та мобільні додатки мають різний користувацький досвід
та стилі, і цей підхід дійсно демонструє, наскільки легко створити два додатки,
які виглядають абсолютно по-різному, але спільно використовують ті самі шари даних
та бізнес-логіки.
