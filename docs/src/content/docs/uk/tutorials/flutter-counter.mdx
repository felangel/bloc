...
Ми розширюємо `MaterialApp`, тому що `CounterApp` _є_ `MaterialApp`. У
більшості випадків ми будемо створювати екземпляри `StatelessWidget` або
`StatefulWidget` та компонувати віджети в `build`, але в цьому випадку немає
віджетів для компонування, тому простіше просто розширити `MaterialApp`.
...
Тільки віджет `Text` обгорнутий у `BlocBuilder`, оскільки це єдиний віджет,
який потрібно перебудовувати у відповідь на зміни стану в `CounterCubit`.
Уникайте зайвого обгортання віджетів, які не потребують перебудови при зміні
стану.
