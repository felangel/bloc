...
If you add an initial event in `BlocProvider.create`, that behavior is
part of the widget tree, not the bloc. Cover it with a widget test
instead of a bloc unit test.
...
Pump the page with a mocked repository and assert the effect of that
event:
...
`pumpApp` is a test helper that wraps the page in a `RepositoryProvider`
and `MaterialApp`. The full test is in the
