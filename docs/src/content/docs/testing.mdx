...
:::note Groups are for organizing individual tests as well as for creating a
context in which you can share a common `setUp` and `tearDown` across all of the
individual tests. :::
...
:::note We can run all of our tests with the `dart test` command. :::
