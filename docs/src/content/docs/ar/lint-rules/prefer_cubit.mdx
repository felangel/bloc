...
هذه القاعدة هي قاعدة أسلوبية بحتة. في بعض الحالات، قد تفضل الفرق توحيد استخدام كائنات `Cubit` فقط في جميع أنحاء تطبيقها بالكامل لضمان الاتساق.
...
لتفعيل قاعدة `prefer_cubit`، أضفها إلى ملف `analysis_options.yaml` الخاص بك ضمن قسم `bloc` > `rules`:
