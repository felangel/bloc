...
من أجل الاتساق، وسهولة الصيانة، وفصل الاهتمامات (separation of concerns)،
**يُفضل** تعريف كائنات `bloc` و `cubit` في ملفات Dart الخاصة بها بدلاً من
تضمينها مباشرة (inlining).

:::tip

فكر في استخدام الأمر `bloc new <component>` من حزمة
[package:bloc_tools](https://pub.dev/packages/bloc_tools) لإنشاء كائنات
bloc/cubit جديدة بسرعة وبشكل متسق.
