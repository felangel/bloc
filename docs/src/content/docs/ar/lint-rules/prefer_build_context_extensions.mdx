...
تم تقديم قاعدة التحقق هذه (lint rule) في الإصدار `0.3.2` من حزمة [`package:bloc_lint`](https://pub.dev/packages/bloc_lint).

:::

## الأساس المنطقي

من أجل الاتساق (consistency)، فضل استخدام امتدادات `BuildContext` مثل `context.read` و `context.watch` و `context.select` بدلاً من `BlocProvider.of` و `RepositoryProvider.of` و `BlocBuilder` أو `BlocSelector`.
...
لتفعيل قاعدة `prefer_build_context_extensions`، أضفها إلى ملف `analysis_options.yaml` الخاص بك تحت المسار `bloc` > `rules`:
