...
يجب استخدام الدوال العامة في كائنات `Cubit` لإخطار `Cubit` وبدء تغييرات الحالة
عبر دالة `emit`. إذا احتاج المُستدعي (caller) إلى الوصول إلى أي معلومات عن
الحالة، فيجب عليه الوصول إليها من خلال خاصية `state` بدلاً من ذلك.

:::note

القواعد التالية ذات صلة ويتم تفعيلها عادةً بالتزامن مع قاعدة
...
`analysis_options.yaml` الخاص بك ضمن قسم `bloc` > `rules`:
