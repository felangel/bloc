---
title: Flutter Todos
description: دليل متعمق لبناء تطبيق مهام (Todos) في فلاتر باستخدام مكتبة bloc.
sidebar:
  order: 6
---
...
في هذا الدليل التعليمي، سنقوم ببناء تطبيق مهام (Todos) باستخدام فلاتر مع مكتبة
Bloc.
...
- [Bloc و Cubit](/bloc-concepts#cubit-vs-bloc) لإدارة حالات الميزات المختلفة.
- [الهيكلية الطبقية](/architecture) لفصل الاهتمامات/المسؤوليات وتسهيل إعادة
  الاستخدام.
- [BlocObserver](/bloc-concepts#blocobserver) لمراقبة تغييرات الحالة.
- [BlocProvider](/flutter-bloc-concepts#blocprovider)، ويدجيت في Flutter يوفر
  Bloc للأبناء.
- [BlocBuilder](/flutter-bloc-concepts#blocbuilder)، ويدجيت في Flutter يتولى
  بناء الويدجيت استجابةً للحالات الجديدة.
- [BlocListener](/flutter-bloc-concepts#bloclistener)، ويدجيت في Flutter ينفذ
  تأثيرات جانبية استجابةً لتغيرات الحالة.
- [RepositoryProvider](/flutter-bloc-concepts#repositoryprovider)، ويدجيت في
  Flutter يوفر مستودع (repository) للأبناء.
- [Equatable](/faqs#when-to-use-equatable) لمنع عمليات إعادة البناء غير
  الضرورية.
- [MultiBlocListener](/flutter-bloc-concepts#multibloclistener)، ويدجيت في
  Flutter يقلل التعشيش عند استخدام عدة BlocListeners.
...
وأخيرًا، نقوم بتثبيت جميع التبعيّات:
...
نقوم بتقسيم المشروع إلى عدة حزم للحفاظ على تبعيات واضحة وصريحة لكل حزمة مع حدود
واضحة تفرض
...
تجمع هذه الطبقة بين مزود بيانات أو أكثر وتطبق "قواعد العمل/الأعمال" على
البيانات. كل مكون في هذه الطبقة يسمى repository وتدير كل repository مجالًا
واحدًا عادةً. الحزم في طبقة الـ repository ينبغي أن تتعامل فقط مع طبقة البيانات.
في هذا المثال، تتكون طبقة الـ repository لدينا من حزمة `todos_repository`.

**طبقة المميزات**

تحتوي هذه الطبقة على جميع الميزات وحالات الاستخدام الخاصة بالتطبيق. تتضمن كل
ميزة عادةً بعض عناصر الواجهة (UI) ومنطق العمل. يجب أن تكون الميزات مستقلة عن
بعضها البعض لتسهيل إضافتها أو إزالتها دون التأثير على بقية قاعدة الكود. ضمن كل
ميزة، يتم إدارة حالة الميزة ومنطق العمل بواسطة blocs. تتفاعل الـ blocs مع بدون
أو مع مستودعات (repositories) متعددة. تستجيب الـ blocs للأحداث (events) وتصدر
...
يُعدّ [المستودع](/architecture#repository) جزءًا من طبقة منطق العمل/الأعمال.
يعتمد المستودع على واحد أو أكثر من مزودي البيانات الذين لا يمتلكون قيمة منطقية
للأعمال، ويجمع واجهات برمجة التطبيقات (APIs) الخاصة بهم في واجهات توفر قيمة
أعمال. بالإضافة إلى ذلك، يساعد وجود طبقة المستودع في تجريد عملية الحصول على
البيانات عن بقية التطبيق، مما يتيح لنا تغيير مكان أو طريقة تخزين البيانات دون
التأثير على باقي أجزاء التطبيق.
...
`App` يغلف ويدير الويجت `RepositoryProvider` الذي يوفر الـ repository لجميع
...
الـ cubit مناسب هنا بسبب بساطة منطق العمل. لدينا دالة واحدة `setTab` لتغيير
التبويب.
...
تمثيل مبسط لشجرة الويجت الخاصة بـ `HomePage` هو:
...
باختبار الويجت `HomeView` بسهولة عن طريق توفير نسخة Mock من `HomeCubit` وتمثيل
...
تيار المهام. راجع قسم [تدفق البيانات](#data-flow) لمزيد من التفاصيل.
...
`todos_view_filter.dart` هو enum يمثل الثلاثة فلاتر للرؤية بالإضافة إلى الدوال
لتطبيق الفلتر.
...
تمثيل مبسط لشجرة الويجت الخاصة بـ `TodosOverviewPage` هو:
...
يقتصر نطاق هذا الـ bloc على الويجتات الموجودة تحت `TodosOverviewPage` فقط.
...
تمثيل مبسط لشجرة الويجت الخاصة بـ `StatsPage` هو:
...
[تدفق البيانات](#data-flow) لمزيد من التفاصيل.
...
- `EditTodoBloc` يتولى منطق العمل بتحديث الـ `TodosRepository`.
...
تمثيل مبسط لشجرة الويجت الخاصة بـ `EditTodosPage` هو:
...
يمكنك العثور على الكود المصدري الكامل لهذا المثال، بما في ذلك اختبارات الوحدة
والـ widget،  
