```markdown
title: بحث GitHub description: دليل شامل حول كيفية بناء تطبيق بحث GitHub
باستخدام Flutter و AngularDart مع bloc. sidebar: order: 9

---

import RemoteCode from '~/components/code/RemoteCode.astro'; import SetupSnippet
from '~/components/tutorials/github-search/SetupSnippet.astro'; import
DartPubGetSnippet from
'~/components/tutorials/github-search/DartPubGetSnippet.astro'; import
FlutterCreateSnippet from
'~/components/tutorials/github-search/FlutterCreateSnippet.astro'; import
FlutterPubGetSnippet from '~/components/tutorials/FlutterPubGetSnippet.astro';
import StagehandSnippet from
'~/components/tutorials/github-search/StagehandSnippet.astro'; import
ActivateStagehandSnippet from
'~/components/tutorials/github-search/ActivateStagehandSnippet.astro';

![advanced](https://img.shields.io/badge/level-advanced-red.svg)

في هذا الدرس التعليمي، سنقوم ببناء تطبيق بحث GitHub باستخدام Flutter و
AngularDart لشرح كيفية مشاركة طبقات البيانات ومنطق العمل بين المشروعين.
...
- [BlocProvider](/flutter-bloc-concepts#blocprovider)، ويدجت Flutter التي توفر
  Bloc لأطفالها.
- [BlocBuilder](/flutter-bloc-concepts#blocbuilder)، ويدجت Flutter التي تدير
  بناء الويدجت استجابة للحالات الجديدة.
...
تتضمن مكتبة البحث المشتركة في GitHub النماذج، مزود البيانات، المستودع، بالإضافة
إلى الـ bloc الذي سيتم مشاركته بين AngularDart و Flutter.
...
يمكنك الاطلاع على نموذج للبيانات التي نحصل عليها من
...
يقوم `GithubClient` ببساطة بإجراء طلب شبكة إلى API بحث المستودعات في GitHub
وتحويل النتيجة إلى إما `SearchResult` أو `SearchResultError` بشكل Future.

:::

:::note

تعتمد طريقة تنفيذ `GithubClient` على الدالة `SearchResult.fromJson`، والتي لم
ننفذها بعد.
...
أنشئ `search_result.dart`، والذي يُمثّل قائمة من `SearchResultItems` تعتمد على
استعلام المستخدم:
...
تعتمد طريقة تنفيذ `SearchResult` على `SearchResultItem.fromJson`، التي لم ننفذها
بعد.
...
مرة أخرى، يعتمد تنفيذ `SearchResultItem` على `GithubUser.fromJson`، التي لم
ننفذها بعد.
...
الآن بعد أن أكملنا تنفيذ `SearchResult` واعتمادياته، سننتقل إلى
`SearchResultError`.
...
لقد انتهينا من `GithubClient`، لذلك سننتقل إلى `GithubCache`، الذي سيكون مسؤولًا
عن
...
سيتولى `GithubCache` مهمة تذكر جميع الاستعلامات السابقة لتجنب إجراء طلبات شبكة
...
مستودع GitHub مسؤول عن خلق طبقة تجريدية بين طبقة البيانات (`GithubClient`) وطبقة
...
يعتمد `GithubRepository` على `GithubCache` و `GithubClient` ويقوم بتجريد التنفيذ
الداخلي. تطبيقنا لا يحتاج لمعرفة كيفية استرجاع البيانات أو مصدرها لأنه لا يهمه
ذلك. يمكننا تغيير طريقة عمل المستودع في أي وقت طالما أننا لم نغير الواجهة
(interface) فليس هناك حاجة لتعديل أي كود يستخدمه.

:::

الآن بعد أن أكملنا طبقة مزود البيانات وطبقة المستودع، نحن جاهزون للانتقال إلى
طبقة منطق الأعمال.

### حدث بحث GitHub

سيتم إعلام الـ Bloc عندما يقوم المستخدم بكتابة اسم مستودع، والذي سنمثله عبر حدث
...
نحن نستخدم الوراثة من [`Equatable`](https://pub.dev/packages/equatable) لتمكين
مقارنة مثيلات `GithubSearchEvent`. بشكل افتراضي، عامل المساواة يعيد true فقط إذا
كانت هذه والمثيل الآخر نفس الكائن.
...
نحن نستخدم الوراثة من [`Equatable`](https://pub.dev/packages/equatable) لتمكين
مقارنة مثيلات `GithubSearchState`. بشكل افتراضي، عامل المساواة يعيد true فقط إذا
كانت هذه والمثيل الآخر نفس الكائن.
...
يقوم `GithubSearchBloc` بتحويل `GithubSearchEvent` إلى `GithubSearchState`
ويعتمد على `GithubRepository`.
...
لـ `GithubSearchEvents`. واحد من الأسباب التي دفعتنا لاستخدام Bloc بدل Cubit هو
للاستفادة من محولات الدفق (stream transformers).

:::

رائع! لقد انتهينا من حزمة `common_github_search`. المنتج النهائي يجب أن يبدو مثل
...
تطبيق Flutter GitHub Search سيكون تطبيق Flutter يعيد استخدام النماذج، مزودي
البيانات، المستودعات (repositories)، وblocs من مكتبة `common_github_search`
لتنفيذ وظيفة البحث في GitHub.

### الإعداد

نبدأ بإنشاء مشروع Flutter جديد داخل مجلد `github_search` على نفس المستوى الذي
...
هذا كل شيء بخصوص إعداد المشروع. بما أن مكتبة `common_github_search` تحتوي على
...
سيكون عنصر `SearchForm` من نوع `StatelessWidget` يعرض عناصر `_SearchBar` و
`_SearchBody`.

أما `_SearchBar` فسيكون من نوع `StatefulWidget` لأنه بحاجة للحفاظ على
`TextEditingController` خاص به ليتتبع ما أدخله المستخدم.

`_SearchBody` هو `StatelessWidget` مسؤول عن عرض نتائج البحث، الأخطاء، مؤشرات
التحميل، وسيكون مستهلكًا لـ `GithubSearchBloc`.

إذا كانت الحالة الحالية هي `SearchStateSuccess`، نعرض `_SearchResults` والتي
سنقوم بتنفيذها لاحقًا.

`_SearchResults` هو `StatelessWidget` يأخذ قائمة من النوع
`List<SearchResultItem>` ويعرضها كمجموعة من عناصر `_SearchResultItem`.

أما `_SearchResultItem` فهو `StatelessWidget` مسؤول عن عرض معلومات نتيجة بحث
واحدة، وكذلك معالجة تفاعل المستخدم والتنقل إلى رابط المستودع عند النقر.
...
يقوم `_SearchBar` بالوصول إلى `GitHubSearchBloc` عبر
`context.read<GithubSearchBloc>()` ويُخطِر الـ bloc بأحداث `TextChanged`.

:::

:::note

يستخدم `_SearchBody` `BlocBuilder` لإعادة البناء استجابة لتغييرات الحالة. ونظرًا
لأن معامل الـ bloc في كائن `BlocBuilder` تم حذفه، فسيقوم `BlocBuilder` تلقائيًا
بالبحث عبر `BlocProvider` وفي السياق الحالي `BuildContext`. يمكنك القراءة أكثر
[هنا.](/flutter-bloc-concepts#blocbuilder)

:::

:::note

نستخدم `ListView.builder` لبناء قائمة قابلة للتمرير من عناصر
`_SearchResultItem`.
...
نستخدم مكتبة [url_launcher](https://pub.dev/packages/url_launcher) لفتح الروابط
الخارجية.
...
يتم إنشاء `GithubRepository` في ملف `main` ويتم حقنه داخل `App`. يتم لفّ
`SearchForm` داخل `BlocProvider` المسؤول عن تهيئة وإغلاق وجعل نسخة
`GithubSearchBloc` متاحة لعناصر واجهة `SearchForm` والأطفال الخاصة به.

:::

هذا كل شيء! لقد طبقنا بنجاح تطبيق بحث GitHub باستخدام Flutter مع مكتبات
[bloc](https://pub.dev/packages/bloc)
و[flutter_bloc](https://pub.dev/packages/flutter_bloc)، وفصلنا بنجاح طبقة العرض
عن طبقة منطق العمل.
...
تطبيق بحث GitHub باستخدام AngularDart سيكون تطبيق AngularDart يعيد استخدام
النماذج، موفري البيانات، المستودعات (repositories)، وbloc من
`common_github_search` لتنفيذ ميزة البحث في GitHub.

### الإعداد

نحتاج أولاً إلى إنشاء مشروع AngularDart جديد داخل مجلد `github_search` على نفس
المستوى مع مجلد `common_github_search`.
...
تمامًا كما في تطبيق Flutter الخاص بنا، سنحتاج لإنشاء `SearchForm` يحتوي على مكون
`SearchBar` و `SearchBody`.

مكون `SearchForm` سيتبع واجهتي `OnInit` و `OnDestroy` لأنه سيحتاج لإنشاء وإغلاق
`GithubSearchBloc`.
...
`SearchBar` هو مكون مسؤول عن استقبال إدخال المستخدم وإعلام `GithubSearchBloc`
بتغييرات النص.
...
`SearchBarComponent` يعتمد على `GitHubSearchBloc` لأنه مسؤول عن إخطار الـ bloc
بحدث `TextChanged`.
...
`SearchBody` هو مكون مسؤول عن عرض نتائج البحث، الأخطاء، ومؤشرات التحميل. سيكون
...
`SearchBodyComponent` يعتمد على `GithubSearchState` والذي يتم توفيره من خلال
`GithubSearchBloc` باستخدام أنبوب الـ bloc في `angular_bloc`.
...
`SearchResults` هو مكون يستقبل قائمة من `List<SearchResultItem>` ويعرضها على
...
`SearchResultItem` هو مكون مسؤول عن عرض المعلومات الخاصة بنتيجة بحث واحدة. وهو
...
هذا كل شيء! لقد قمنا الآن بنجاح بتنفيذ تطبيق بحث GitHub باستخدام AngularDart مع
حزمتي `bloc` و `angular_bloc`، وفصلنا بنجاح طبقة العرض عن منطق العمل/الأعمال.

يمكنك إيجاد الشيفرة المصدرية كاملة
[هنا](https://github.com/felangel/bloc/tree/master/examples/github_search/angular_github_search).

## الملخص

في هذا الدرس، قمنا بإنشاء تطبيق Flutter و AngularDart مع مشاركة جميع النماذج
(models)، مزودي البيانات (data providers)، وbloc بين الاثنين.

الشيء الوحيد الذي اضطررنا لكتابته مرتين فعليًا هو طبقة العرض (واجهة المستخدم)،
وهو أمر رائع من حيث الكفاءة وسرعة التطوير. بالإضافة إلى ذلك، من الشائع جدًا أن
تمتلك تطبيقات الويب وتطبيقات الجوال تجارب مستخدم وأنماط تصميم مختلفة، وتُظهر هذه
الطريقة مدى سهولة بناء تطبيقين يبدوان مختلفين تمامًا بينما يشتركان في نفس بيانات
ومنطق العمل.
