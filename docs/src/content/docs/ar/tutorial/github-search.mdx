---
title: بحث GitHub
description: دليل شامل حول كيفية بناء تطبيق بحث GitHub باستخدام Flutter و AngularDart مع bloc.
sidebar:
  order: 9
---
...
- استخدام محول أحداث مخصص `EventTransformer` مع [`bloc_concurrency`](https://pub.dev/packages/bloc_concurrency).
...
الـ `GithubClient` هو المسؤول عن توفير البيانات الخام من [واجهة برمجة تطبيقات GitHub](https://developer.github.com/v3/).

:::note

يمكنك الاطلاع على نموذج للبيانات التي نحصل عليها من [هنا](https://api.github.com/search/repositories?q=dartlang).
...
يقوم `GithubClient` ببساطة بإجراء طلب شبكة إلى API بحث المستودعات في GitHub وتحويل النتيجة إلى إما `SearchResult` أو `SearchResultError` بشكل Future.

:::

:::note

تعتمد طريقة تنفيذ `GithubClient` على الدالة `SearchResult.fromJson`، والتي لم ننفذها بعد.
...
تعتمد طريقة تنفيذ `SearchResult` على `SearchResultItem.fromJson`، التي لم ننفذها بعد.
...
مرة أخرى، يعتمد تنفيذ `SearchResultItem` على `GithubUser.fromJson`، التي لم ننفذها بعد.
...
لقد انتهينا من `GithubClient`، لذلك سننتقل إلى `GithubCache`، الذي سيكون مسؤولًا عن [التخزين المؤقت للنتائج (Memoization)](https://en.wikipedia.org/wiki/Memoization) كوسيلة لتحسين الأداء.
...
أنشأنا مُحوّل أحداث مخصص `EventTransformer` لـ [تأخير التنفيذ (Debounce)](https://pub.dev/documentation/stream_transform/latest/stream_transform/RateLimit/debounce.html) لـ `GithubSearchEvents`. واحد من الأسباب التي دفعتنا لاستخدام Bloc بدل Cubit هو للاستفادة من محولات الدفق (stream transformers).

:::

رائع! لقد انتهينا من حزمة `common_github_search`. المنتج النهائي يجب أن يبدو مثل [هذا](https://github.com/felangel/bloc/tree/master/examples/github_search/common_github_search).
...
نحن نستخدم `BlocBuilder` لإعادة بناء الواجهة في كل مرة تتغير فيها حالة `GithubSearchBloc`.
...
يمكن العثور على المصدر الكامل [هنا](https://github.com/felangel/bloc/tree/master/examples/github_search).