...
structure). يتيح لنا هذا النمط توسيع المشروع عبر ميزات مستقلة بذاتها.
...
`jsonplaceholder` هي REST API عبر الإنترنت تقدم بيانات وهمية؛ وهي
...
من class declaration فقط، يمكننا معرفة أن `PostBloc` يستقبل `PostEvents`
كمدخلات ويخرج `PostStates`.
...
في Widget `PostsPage`، نستخدم `BlocProvider` لإنشاء وتوفير instance من `PostBloc`
للشجرة الفرعية. كما نضيف حدث `PostFetched` بحيث يطلب التطبيق الدفعة الأولية من
المنشورات عند التحميل.
...
`PostsList` هي `StatefulWidget` لأنها تحتاج للاحتفاظ بـ `ScrollController`.
في `initState`، نضيف listener إلى `ScrollController` حتى نستجيب لأحداث التمرير.
كما نصل إلى instance من `PostBloc` عبر
`context.read<PostBloc>()`.

:::

في خطوة البناء، تعيد `build method` لدينا `BlocBuilder`. و`BlocBuilder` هي Widget
من Flutter ضمن
[حزمة flutter_bloc](https://pub.dev/packages/flutter_bloc) تتولى بناء ويدجت
استجابةً لحالات bloc الجديدة. وعند تغيّر حالة `PostBloc`، سيتم استدعاء `builder`
بالحالة الجديدة `PostState`.
...
هذا كل ما في الأمر! نجحنا الآن في تنفيذ قائمة لانهائية في Flutter باستخدام
