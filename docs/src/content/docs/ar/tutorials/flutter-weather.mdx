...
في هذا الدرس، سنقوم ببناء تطبيق طقس باستخدام Flutter يوضح كيفية إدارة عدة cubits
لتنفيذ التخصيص الديناميكي للثيمات، والسحب للتحديث، والعديد من الميزات الأخرى.
سيعتمد تطبيق الطقس على جلب بيانات الطقس الحية من واجهة برمجة التطبيقات العامة
OpenMeteo، وسنشرح كيفية فصل تطبيقنا إلى طبقات (البيانات، المستودع، منطق العمل،
وطبقة العرض).
...
- [BlocProvider](/flutter-bloc-concepts#blocprovider)، ويدجت Flutter التي توفر
  bloc لأبنائها.
- [BlocBuilder](/flutter-bloc-concepts#blocbuilder)، ويدجت Flutter التي تتولى
  بناء الودجت استجابةً للحالات الجديدة.
- تجنب عمليات إعادة البناء غير الضرورية باستخدام
  [Equatable](/faqs#when-to-use-equatable).
- [RepositoryProvider](/flutter-bloc-concepts#repositoryprovider)، ويدجت Flutter
  التي توفر repository لأبنائها.
- [BlocListener](/flutter-bloc-concepts#bloclistener)، ويدجت Flutter التي تستدعي
  كود المستمع استجابةً لتغيرات الحالة في الـ bloc.
- [MultiBlocProvider](/flutter-bloc-concepts#multiblocprovider)، ويدجت Flutter
  التي تجمع عدة BlocProvider في واحد.
- [BlocConsumer](/flutter-bloc-concepts#blocconsumer)، ويدجت Flutter التي توفر
  builder و listener للاشتراك في حالات جديدة.
- [HydratedBloc](https://github.com/felangel/bloc/tree/master/packages/hydrated_bloc)
...
مع كلا طلبات الـ API، بما في ذلك السيناريوهات الحدية (Edge Cases).
...
الهدف من طبقة الريبو لدينا هو تجريد طبقة البيانات وتسهيل الاتصال مع طبقة الـ
bloc. عند القيام بذلك، يعتمد بقية الكود الأساسي لدينا فقط على الدوال المعروضة من
خلال طبقة الريبو بدلاً من الاعتماد على تنفيذات مزود البيانات المحددة. هذا يسمح
لنا بتغيير مزودي البيانات دون التأثير على أي كود في مستوى التطبيق. على سبيل
المثال، إذا قررنا الانتقال بعيدًا عن واجهة برمجة التطبيقات الخاصة بالطقس التي
نستخدمها حالياً، يجب أن نتمكن من إنشاء عميل API جديد واستبداله دون الحاجة لتعديل
واجهة برمجة التطبيقات العامة للريبو أو طبقات التطبيق.
...
تمامًا كما هو الحال مع طبقة البيانات، من الضروري اختبار طبقة الريبو لضمان صحة
منطق الأعمال/المنطق على مستوى المجال. لاختبار `WeatherRepository`، سنستخدم مكتبة
...
- استخدام [equatable](https://pub.dev/packages/equatable) يتيح لمثيلات حالة
  ...
  إعادة بناء. هذا يضمن أن شجرة الـ widget ستُعاد بناؤها فقط عند الضرورة للحفاظ
  على الأداء سريعًا ومتجاوبًا.
...
سنبدأ بـ `WeatherPage` التي تستخدم `BlocProvider` من أجل توفير نسخة من
`WeatherCubit` لشجرة الويدجت.
...
ستلاحظ أن هذه الصفحة تعتمد على ويدجت `SettingsPage` و `SearchPage`، اللتين
سننشئهما بعد ذلك.
...
ستعرض التطبيق شاشات مختلفة اعتمادًا على الحالات الأربع المحتملة لـ
`WeatherCubit`.
...
ويدجت `app.dart` يتولى بناء عرض `WeatherPage` الذي أنشأناه سابقًا ويستخدم
...
و `MockCubits` التي تجعل من السهل اختبار واجهة المستخدم. يمكننا تقمص حالات الـ
...
يمكن العثور على الشفرة المصدرية الكاملة لهذا المثال، بما في ذلك اختبارات الوحدة
والاختبارات الخاصة بالـ widget،  
