...
description:
  دليل متعمّق لبناء مسار تسجيل الدخول في Flutter باستخدام مكتبة Bloc.
sidebar:
  order: 4
---
...
تسترجع المستخدم الحالي. نحن هنا نستخدم stubbing، لكن في
...
الفصل (decoupling) يسمح لنا باختبار كل من Widget `App` و`AppView` بسهولة
لاحقًا.
...
افتراضيًا، `BlocProvider` يكون lazy ولا يستدعي `create` إلا عند أول وصول إلى
الـ Bloc. وبما أن `AuthenticationBloc` يجب أن يشترك دائمًا في stream
...
يتولى `LoginForm` إخطار `LoginBloc` بأحداث المستخدم ويستجيب أيضًا للتغيرات
