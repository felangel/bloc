...
description: دليل متعمق حول كيفية بناء مسار تسجيل دخول في Flutter باستخدام مكتبة bloc.  
sidebar:  
  order: 4  
---
...
- [BlocProvider](/flutter-bloc-concepts#blocprovider)، ويدجت Flutter يقوم بتوفير bloc للأطفال (العناصر الفرعية) الخاصة به.
- إضافة الأحداث باستخدام [context.read](/flutter-bloc-concepts#contextread).
- تجنب إعادة البناء غير الضرورية باستخدام [Equatable](/faqs#when-to-use-equatable).
- [RepositoryProvider](/flutter-bloc-concepts#repositoryprovider)، ويدجت Flutter يقوم بتوفير repository للأطفال الخاصة به.
- [BlocListener](/flutter-bloc-concepts#bloclistener)، ويدجت Flutter يقوم بتنفيذ كود المستمع استجابة لتغيرات الحالة في الـ bloc.
- تحديث واجهة المستخدم بناءً على جزء من حالة الـ bloc باستخدام [context.select](/flutter-bloc-concepts#contextselect).
...
حزمة `package:authentication_repository` ستكون حزمة Dart نقية بدون أي تبعيات خارجية.
...
بما أننا ندير `StreamController` داخليًا، يتم عرض طريقة `dispose` ليتمكن المستخدم من إغلاق الـ controller عندما لا يكون مطلوبًا بعد الآن.

:::

أخيرًا، نحتاج إلى إنشاء الملف
`packages/authentication_repository/lib/authentication_repository.dart` والذي سيحتوي على الصادرات العامة (public exports):
...
يتم استخدام [`package:equatable`](https://pub.dev/packages/equatable) لتمكين مقارنة القيم داخل كائن `User`.
...
استخدم [امتداد VSCode](https://marketplace.visualstudio.com/items?itemName=FelixAngelov.bloc) 
أو [إضافة IntelliJ](https://plugins.jetbrains.com/plugin/12129-bloc) لإنشاء blocs تلقائيًا.
...
بعد أن اطلعنا على تنفيذ `AuthenticationEvent` و`AuthenticationState`، دعونا نلقي نظرة على `AuthenticationBloc`.
...
يعتمد `AuthenticationBloc` على كل من `AuthenticationRepository` و`UserRepository`، ويحدد الحالة الابتدائية كـ `AuthenticationState.unknown()`.
...
إذا تم حذف `onError`، تُعتبر أي أخطاء في تدفق `status` غير معالجة، وسيتم رميها بواسطة `onEach`، مما يؤدي إلى إلغاء الاشتراك في التدفق.

:::

:::tip

يُعد [`BlocObserver`](/bloc-concepts/#blocobserver-1) أداة ممتازة لتسجيل أحداث الـ Bloc والأخطاء وتغيرات الحالة، خصوصًا في سياق التحليلات وتقرير الأعطال.
...
تم تقسيم `app.dart` إلى جزأين: `App` و `AppView`. يتحمل `App` مسؤولية إنشاء/توفير `AuthenticationBloc` الذي سيتم استهلاكه من قبل `AppView`. هذا الانفصال/الغير مترابط يسمح لنا باختبار كل من ويدجت `App` و`AppView` بسهولة لاحقًا.

:::

:::note

يُستخدم `RepositoryProvider` لتوفير نسخة واحدة من `AuthenticationRepository` لكامل التطبيق، وهو ما سيكون مفيدًا لاحقًا.
...
نستخدم [`package:formz`](https://pub.dev/packages/formz) لإنشاء نماذج قابلة لإعادة الاستخدام وموحدة لـ `username` و`password`.
...
نماذج `Username` و `Password` تُستخدم كجزء من `LoginState`، والحالة (status) هي أيضًا جزء من [package:formz](https://pub.dev/packages/formz).
...
يُستخدم `context.read<AuthenticationRepository>()` للبحث عن نسخة من `AuthenticationRepository` عبر `BuildContext`.
...
`context.select((AuthenticationBloc bloc) => bloc.state.user.id)` يُحدث تحديثات تلقائية في حال تغيّر معرف المستخدم.
