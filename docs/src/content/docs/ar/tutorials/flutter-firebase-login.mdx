...
- [RepositoryProvider](/ar/flutter-bloc-concepts#repositoryprovider)، وهي ويدجت
  من Flutter توفر مستودع (Repository) لأبنائها.
- [BlocListener](/ar/flutter-bloc-concepts#bloclistener)، وهي ويدجت من Flutter
  تستدعي كود المستمع (listener) استجابةً لتغيرات الحالة في الكتلة (Bloc).
- إضافة الأحداث باستخدام
  [context.read](/ar/flutter-bloc-concepts#contextselect).
...
تمامًا كما في [دليل تسجيل الدخول](/ar/tutorials/flutter-login)، سنقوم بإنشاء حزم
...
تمامًا كما في [دليل تسجيل الدخول](/ar/tutorials/flutter-login)، سيوفر ملف
...
يعد [`BlocObserver`](/ar/bloc-concepts/#blocobserver) رائعًا لتسجيل أحداث Bloc
