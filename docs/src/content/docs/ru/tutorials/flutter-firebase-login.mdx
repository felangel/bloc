...
description:
  Подробное руководство по созданию потока входа Flutter с использованием bloc и Firebase.
sidebar:
  order: 7
---
...
Так же, как в [руководстве по входу](/tutorials/flutter-login), we're going to
...
that can occur в каталоге data layer and exposes a clean, simple set of errors that
...
local assets. Create an `assets` directory в каталоге root of your project and add
the
...
Так же, как в [руководстве по входу](/tutorials/flutter-login), our `app.dart` will
...
`user` Stream in order to emit new states in response to changes в каталоге current
user.
...
events, errors, and state changes especially в каталоге context analytics and crash
reporting.
...
updated which will trigger a state change в каталоге `AuthenticationBloc` and will
result в каталоге `AppView` pushing the `HomePage` route onto the navigation stack.
...
feature. In this case a simple `Avatar` widget is exported and used withв каталоге
`HomePage`.
