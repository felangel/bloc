...
Для согласованности и ради явности предпочтительно использовать напрямую
базовые методы вместо расширений `BuildContext`. Это также полезно
для тестирования, поскольку невозможно замокать метод расширения.
...
**Избегайте** использования расширений `BuildContext` для взаимодействия с экземплярами `Bloc` или `Cubit`.
