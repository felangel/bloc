...
Это правило является чисто стилистическим. В некоторых случаях команды могут предпочесть
стандартизировать использование только экземпляров `Cubit` во всем приложении
для согласованности.
...
Чтобы включить правило `prefer_cubit`, добавьте его в `analysis_options.yaml` в разделе
`bloc` > `rules`:
