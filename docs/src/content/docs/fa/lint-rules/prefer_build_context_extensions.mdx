...
ترجیح دهید از اکستنشن‌های `BuildContext` برای دسترسی به یک نمونهٔ `Bloc` یا `Repository` استفاده کنید.
...
برای حفظ یکپارچگی، ترجیح دهید از اکستنشن‌های `BuildContext` مانند `context.read`،
`context.watch` و `context.select` به‌جای `BlocProvider.of`، `RepositoryProvider.of`،
`BlocBuilder` یا `BlocSelector` استفاده کنید.
...
## مثال‌ها
**اجتناب کنید**
از استفاده از `BlocProvider.of<T>(context)` برای دسترسی به یک نمونهٔ `Bloc`
