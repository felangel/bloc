...
:::note `package:authentication_repository` will be a pure Dart package without
...
:::note Since we are maintaining a `StreamController` internally, a `dispose`
...
:::note [`package:equatable`](https://pub.dev/packages/equatable) is used to
enable value comparisons of the `User` object. :::
...
:::caution If `onError` is omitted, any errors on the `status` stream are
considered unhandled, and will be thrown by `onEach`. As a result, the
subscription to the `status` stream will be canceled. :::
...
:::note `app.dart` is split into two parts `App` and `AppView`. `App` is
responsible for creating/providing the `AuthenticationBloc` which will be
consumed by the `AppView`. This decoupling will enable us to easily test both
the `App` and `AppView` widgets later on. :::
...
:::tip `SplashPage` exposes a static `Route` which makes it very easy to
navigate to via `Navigator.of(context).push(SplashPage.route())`; :::
...
:::note The `Username` and `Password` models are used as part of the
`LoginState` and the status is also part of
[package:formz](https://pub.dev/packages/formz). :::
...
:::note `context.read<AuthenticationRepository>()` is used to lookup the
instance of `AuthenticationRepository` via the `BuildContext`. :::
...
:::note `context.select((AuthenticationBloc bloc) => bloc.state.user.id)` will
