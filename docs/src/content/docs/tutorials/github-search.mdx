...
:::note The `common_github_search` directory will contain the shared library.
:::
...
:::note You can see a sample of what the data we get back will look like
...
:::note Our `GithubClient` is simply making a network request to Github's
Repository Search API and converting the result into either a `SearchResult` or
`SearchResultError` as a `Future`. :::

:::note The `GithubClient` implementation depends on `SearchResult.fromJson`,
...
:::note The `SearchResult` implementation depends on
`SearchResultItem.fromJson`, which we have not yet implemented. :::

:::note We aren't including properties that aren't going to be used in our
model. :::
...
:::note Again, the `SearchResultItem` implementation dependes on
`GithubUser.fromJson`, which we have not yet implemented. :::
...
:::note The `GithubRepository` has a dependency on the `GithubCache` and the
...
:::note We extend [`Equatable`](https://pub.dev/packages/equatable) so that we
can compare instances of `GithubSearchEvent`. By default, the equality operator
...
:::note We extend [`Equatable`](https://pub.dev/packages/equatable) so that we
can compare instances of `GithubSearchState`. By default, the equality operator
returns true if and only if this and other are the same instance. :::
...
:::note Our `GithubSearchBloc` converts `GithubSearchEvent` to
`GithubSearchState` and has a dependency on the `GithubRepository`. :::

:::note We create a custom `EventTransformer` to
...
:::note We are including our newly created `common_github_search` library as a
dependency. :::
...
:::note `_SearchBar` accesses `GitHubSearchBloc` via
`context.read<GithubSearchBloc>()` and notifies the bloc of `TextChanged`
events. :::

:::note `_SearchBody` uses `BlocBuilder` in order to rebuild in response to
state changes. Since the bloc parameter of the `BlocBuilder` object was omitted,
...
:::note We use `ListView.builder` in order to construct a scrollable list of
`_SearchResultItem`. :::

:::note We use the [url_launcher](https://pub.dev/packages/url_launcher) package
...
Our `SearchForm` is wrapped in a `BlocProvider` which is responsible for
...
:::note You can install `stagehand` via:
...
:::note The `GithubSearchBloc` is created and closed by the
`SearchFormComponent`. :::
...
:::note `SearchBarComponent` has a dependency on `GitHubSearchBloc` because it
is responsible for notifying the bloc of `TextChanged` events. :::
...
:::note `SearchBodyComponent` has a dependency on `GithubSearchState` which is
provided by the `GithubSearchBloc` using the `angular_bloc` bloc pipe. :::
...
:::note We use `ngFor` in order to construct a list of `SearchResultItem`
components. :::
...
:::note We're creating the `GithubRepository` in the `AppComponent` and
injecting it into the `SearchForm` component. :::
