...
:::note jsonplaceholder is an online REST API which serves fake data; it's very
useful for building prototypes. :::
...
:::note In our url we specified the start and limit as query parameters to the
GET request. :::
...
can compare `Posts`. Without this, we would need to manually change our class to
...
:::note We implemented `copyWith` so that we can copy an instance of
`PostSuccess` and update zero or more properties conveniently (this will come in
handy later). :::
...
:::note Just from the class declaration we can tell that our PostBloc will be
taking PostEvents as input and outputting PostStates. :::
...
:::note Passing a `transformer` to `on<PostFetched>` allows us to customize how
...
:::note `PostsList` is a `StatefulWidget` because it will need to maintain a
...
:::caution We need to remember to clean up after ourselves and dispose of our
...
:::note A `Transition` consists of the current state, the event, and the next
state. :::
...
:::note All we need to do is extend `BlocObserver` and override the
`onTransition` method. :::

Now every time a Bloc `Transition` occurs we can see the transition printed to
the console.

:::note In practice, you can create different `BlocObservers` and because every
state change is recorded, we are able to very easily instrument our applications
and track all user interactions and state changes in one place! :::
