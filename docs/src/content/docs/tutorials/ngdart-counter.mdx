...
:::note Just from the class declaration we can tell that our `CounterBloc` will
be taking `CounterEvents` as input and outputting integers. :::
...
:::note We are able to access the `CounterBloc` instance using AngularDart's
dependency injection system. Because we have registered it as a `Provider`,
AngularDart can properly resolve `CounterBloc`. :::
...
:::note We are using the `BlocPipe` so that we can display our `CounterBloc`
state as it is updated. :::
