...
:::note The `authentication_repository` package will be exposing an
...
:::note It's completely up to you to define what a user needs to look like in
...
:::note The `User` class is extending
[equatable](https://pub.dev/packages/equatable) in order to override equality
...
:::note The `AuthenticationRepository` is also responsible for handling
low-level errors that can occur in the data layer and exposes a clean, simple
set of errors that align with the domain. :::
...
:::caution Remember to update the `google-services.json` on Android and the
...
:::note We are depending on the `authentication_repository` package via path
which will allow us to iterate quickly while still maintaining a clear
separation. :::
...
:::note We're injecting a single instance of `AuthenticationRepository` into the
`App` and it is an explicit constructor dependency. :::
...
:::caution If `onError` is omitted, any errors on the user stream are considered
unhandled, and will be thrown by `onEach`. As a result, the subscription to the
user stream will be canceled. :::

:::tip A [`BlocObserver`](/bloc-concepts/#blocobserver-1) is great for logging
Bloc events, errors, and state changes especially in the context analytics and
crash reporting. :::
...
:::tip It's very important to keep the creation of blocs/cubits separate from
where they are consumed. This will allow you to easily inject mock instances and
test your view in isolation. :::
...
:::note We used a `Cubit` instead of a `Bloc` here because the `LoginState` is
fairly simple and localized. Even without events, we can still have a fairly
good sense of what happened just by looking at the changes from one state to
...
:::note Just as in the `LoginCubit`, the `SignUpCubit` has a dependency on the
...
:::note A `widgets` directory was created alongside the `view` directory within
the `home` feature for reusable components that are specific to that particular
feature. In this case a simple `Avatar` widget is exported and used within the
`HomePage`. :::

:::note When the logout `IconButton` is tapped, an
`AuthenticationLogoutRequested` event is added to the `AuthenticationBloc` which
signs the user out and navigates them back to the `LoginPage`. :::
