...
The `AppState` consists of an `AppStatus` and a `User`. A factory constructor `fromUser` is exposed which transforms `User` to the corresponding `AuthenticationStatus`.
