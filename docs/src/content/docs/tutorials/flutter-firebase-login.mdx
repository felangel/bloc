...
Before building the authentication repository, we'll create a tiny local `cache`
package. This is **not** a Flutter or Firebase caching solution — it's a simple
in-memory key/value store used by `AuthenticationRepository` to keep the current
`User` readily available without re-querying Firebase on every read.
...
`currentUser` reads from it (falling back to `User.empty` when nothing is cached
yet). That keeps the rest of the app able to access the signed-in user
