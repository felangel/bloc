...
:::note[¿Qué cambió?] En bloc_test v10.0.0, la API `blocTest` ya no está
estrechamente acoplada a `BlocBase`. :::
...
:::note[¿Qué cambió?] En hydrated_bloc v10.0.0, se añadió soporte para compilar
a WebAssembly (wasm). :::
...
:::note[¿Qué cambió?] En bloc v9.0.0, todas las APIs previamente obsoletas
fueron eliminadas. :::
...
:::note[¿Qué cambió?] En bloc v9.0.0, se introdujo una nueva interfaz central
...
:::note[¿Qué cambió?] En hydrated_bloc v9.0.0, `HydratedBlocOverrides` fue
eliminado en favor de la API `HydratedBloc.storage`. :::
...
:::note[¿Qué cambió?] En bloc v8.1.0, `BlocOverrides` fue deprecado en favor de
las APIs `Bloc.observer` y `Bloc.transformer`. :::
...
:::note[¿Qué cambió?] En bloc v8.0.0, `Bloc.observer` y `Bloc.transformer`
fueron eliminados en favor de la API `BlocOverrides`. :::
...
:::note[¿Qué cambió?] En bloc v8.0.0, `BlocUnhandledErrorException` se eliminó.
Además, cualquier excepción no capturada siempre se reporta a `onError` y se
vuelve a lanzar (independientemente del modo de depuración o lanzamiento). La
API `addError` informa errores a `onError`, pero no trata los errores reportados
como excepciones no capturadas. :::
...
:::note[¿Qué cambió?] En bloc v8.0.0, `BlocObserver` se convirtió en una clase
`abstract`, lo que significa que no se puede instanciar una instancia de
`BlocObserver`. :::
...
:::note[¿Qué cambió?] En bloc v8.0.0, llamar a `add` en un bloc cerrado
resultará en un `StateError`. :::
...
:::note[¿Qué cambió?] En bloc v8.0.0, llamar a `emit` dentro de un bloc cerrado
resultará en un `StateError`. :::
...
:::note[¿Qué cambió?] En bloc v8.0.0, todas las APIs previamente obsoletas
fueron eliminadas. :::
...
:::note[¿Qué cambió?] En bloc_test v9.0.0, los desarrolladores ya no necesitan
llamar explícitamente a `registerFallbackValue` al usar `MockBloc` o
`MockCubit`. :::
...
:::note[¿Qué cambió?] En hydrated_bloc v8.0.0, `HydratedBloc.storage` fue
eliminado en favor de la API `HydratedBlocOverrides`. :::
...
:::note[¿Qué cambió?] En bloc v7.2.0, `mapEventToState` fue deprecado en favor
de `on<Event>`. `mapEventToState` será eliminado en bloc v8.0.0. :::
...
:::note Cada `EventHandler` registrado funciona de manera independiente, por lo
que es importante registrar manejadores de eventos según el tipo de
transformador que desees aplicar. :::
...
:::note[¿Qué cambió?] En bloc v7.2.0, `transformEvents` fue deprecado en favor
de la API `EventTransformer`. `transformEvents` será eliminado en bloc v8.0.0.
:::
...
:::tip Consulta
[package:bloc_concurrency](https://pub.dev/packages/bloc_concurrency) para un
conjunto de transformadores de eventos personalizados y con opiniones. :::
...
:::note[¿Qué cambió?] En bloc v7.2.0, `transformTransitions` fue deprecado en
favor de sobrescribir la API `stream`. `transformTransitions` será eliminado en
bloc v8.0.0. :::
...
:::note Usar `context.watch` en la raíz del método `build` resultará en que todo
el widget se reconstruya cuando cambie el estado del bloc. Si no es necesario
...
:::note `emit` solo es visible para pruebas y nunca debe usarse fuera de las
pruebas. :::
...
:::note Idealmente, no debería haber ninguna funcionalidad o características
orientadas al usuario manejadas dentro de `BlocObserver`. :::
