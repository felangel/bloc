...
:::note Todos los widgets exportados por el paquete `flutter_bloc` se integran
tanto con instancias de `Cubit` como de `Bloc`. :::
...
:::note `context.read<T>()` no escucha a `T` -- si el `Object` proporcionado del
tipo `T` cambia, `context.read` no activará una reconstrucción del widget. :::
...
:::caution Usa `BlocBuilder` o `context.watch` en su lugar para reconstruir en
...
:::caution `context.watch` solo es accesible dentro del método `build` de una
clase `StatelessWidget` o `State`. :::
...
:::caution Usar `context.watch` en la raíz del método `build` resultará en que
todo el widget se reconstruya cuando el estado del bloc cambie. :::
...
:::caution Usar `context.select` en la raíz del método `build` resultará en que
todo el widget se reconstruya cuando la selección cambie. :::
