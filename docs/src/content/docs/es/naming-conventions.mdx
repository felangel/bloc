...
:::note La clase base del evento debe llamarse: `BlocSubject` + `Event`. :::
...
:::note Los estados iniciales deben seguir la convención: `BlocSubject` +
`Initial`. :::
...
:::note La clase base del estado siempre debe llamarse: `BlocSubject` + `State`.
:::
