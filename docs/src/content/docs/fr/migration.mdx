...
description: Migrez vers la dernière version stable de Bloc.
---
...
#### ❗✨ Découpler `blocTest` de `BlocBase`

:::note[Qu'est-ce qui a changé ?]

Dans bloc_test v10.0.0, l'API `blocTest` n'est plus étroitement couplée à
`BlocBase`.
...
para una mayor flexibilidad y reutilisezción. Auparavant cela n'était pas possible
parce que `BlocBase` implémentait `StateStreamableSource`, ce qui n'était pas
suffisant pour `blocTest` en raison de la dépendance interne à l'API `emit`.
...
En hydrated_bloc v10.0.0, un support pour compiler en WebAssembly (wasm) a été ajouté.

:::

##### Justification

Auparavant il n'était pas possible de compiler des applications en wasm lors de l'utilisation de
...
Dans bloc v9.0.0, toutes les APIs précédemment obsolètes ont été supprimées.

:::

##### Résumé

- `BlocOverrides` supprimé en faveur de `Bloc.observer` y `Bloc.transformer`

#### ❗✨ Introduire une Nouvelle Interface `EmittableStateStreamableSource`

:::note[Qu'est-ce qui a changé ?]

Dans bloc v9.0.0, une nouvelle interface centrale a été introduite
...
#### ✨ Réintroduire l'API `HydratedBloc.storage`

:::note[Qu'est-ce qui a changé ?]

En hydrated_bloc v9.0.0, `HydratedBlocOverrides` a été supprimé en favor de la
...
Consultez la
[justification pour réintroduire les remplacements de Bloc.observer et Bloc.transformer](/fr/migration#-réintroduire-les-apis-blocobserver-et-bloctransformer).
...
#### ✨ Réintroduire les APIs `Bloc.observer` et `Bloc.transformer`

:::note[Qu'est-ce qui a changé ?]

Dans bloc v8.1.0, `BlocOverrides` a été déprécié en favor de las APIs
...
cependant, en aplicaciones Flutter la nouvelle API causó más problemas de los que
resolvió.
...
Aunque no fue la razón principal para estos cambios, l'API `BlocOverrides`
...
efecto, l'API `BlocOverrides` requería que los desarrolladores tuvieran un
...
Par exemple, muchos desarrolladores tendrían algo como:
...
El código précédent, aunque parece inofensivo, puede llevar a muchos errores
...
bifurcar/manipular Zonas (especialmente al exécutezr pruebas) lo que puede llevar
...
transición a l'API `BlocOverrides` llevó al descubrimiento de varios
...
#### ❗✨ Introducir nouvelle API `BlocOverrides`
...
La API précédent utilisezda para sobrescribir el `BlocObserver` y
`EventTransformer` par défauts dependía de un singleton global tanto para el
...
- Tener sobrescrituras de `BlocObserver` o `EventTransformer` limitadas a un
  ...
  - Si un paquete dependía de `package:bloc` y registraba su propio
    `BlocObserver`, cualquier consumidor dle package tendría que sobrescribir el
    `BlocObserver` dle package o informar al `BlocObserver` dle package.

También era más difícil de probar en raison del estado global compartido entre las
pruebas.
...
`Zone` específica au lieu de depender de un singleton global mutable.
...
para la zona, usarán los valores par défauts internos existentes (sin cambio
...
BlocOverrides.runZoned(
  () {
    ...
    // y usan eventTransformerA como el transformador par défaut.
    // ...
    ...
    BlocOverrides.runZoned(
      () {
        ...
        // y usan eventTransformerB como el transformador par défaut.
        // ...
      },
...
Dans bloc v8.0.0, `BlocUnhandledErrorException` se eliminó. Además, cualquier
...
aplicación se exécutezba en modo de depuración o lanzamiento. Además, los errores
...
usar l'API `addError` (específicamente al escribir pruebas unitarias).
...
errores aún se reportan a `onError`, cependant, solo las excepciones no
...
Dans bloc v8.0.0, `BlocObserver` se convirtió en una clase `abstract`, lo que
...
par défaut de l'API son operaciones nulas, `BlocObserver` es maintenant una
clase `abstract` para comunicar claramente que la classe está destinada a ser
...
Dans bloc v8.0.0, llamar a `add` en un bloc cerrado resultará en un `StateError`.
...
Dans bloc v8.0.0, llamar a `emit` dentro de un bloc cerrado resultará en un
`StateError`.
...
Dans bloc v8.0.0, toutes les APIs précédemment obsolètes ont été supprimées.

:::

##### Résumé

- `mapEventToState` supprimé en faveur de `on<Event>`
- `transformEvents` supprimé en faveur de l'API `EventTransformer`
- `TransitionFunction` typedef supprimé en faveur de l'API `EventTransformer`
- `listen` supprimé en faveur de `stream.listen`

### `package:bloc_test`

#### ✨ `MockBloc` y `MockCubit` ya no nécessiten `registerFallbackValue`

:::note[Qu'est-ce qui a changé ?]

Dans bloc_test v9.0.0, los desarrolladores ya no necesitan llamar explícitamente a
...
`registerFallbackValue` solo es nécessaire cuando se usa el matcher `any()` de
`package:mocktail` para un tipo personalizado. Auparavant,
`registerFallbackValue` era nécessaire para cada `Event` y `State` al usar
...
#### ❗✨ Introducir nouvelle API `HydratedBlocOverrides`

:::note[Qu'est-ce qui a changé ?]

En hydrated_bloc v8.0.0, `HydratedBloc.storage` a été supprimé en favor de l'API
...
#### ✨ Introducir nouvelle API `on<Event>`

:::note[Qu'est-ce qui a changé ?]

Dans bloc v7.2.0, `mapEventToState` a été déprécié en favor de `on<Event>`.
...
siempre es obvio cuál será la valeur de `state` cuando se trata de generadores
...
También puedes sobrescribir el `EventTransformer` par défaut para todos los
...
#### ✨ Introducir nouvelle API `EventTransformer`

:::note[Qu'est-ce qui a changé ?]

Dans bloc v7.2.0, `transformEvents` a été déprécié en favor de l'API
...
El `EventTransformer` par défaut procesa todos los eventos concurrentemente
y se ve algo así:
...
para un conjunto de transformadores de eventos personnalisés y con opiniones.
...
#### ⚠️ Marcar como obsoleta l'API `transformTransitions`

:::note[Qu'est-ce qui a changé ?]

Dans bloc v7.2.0, `transformTransitions` a été déprécié en favor de sobrescribir la
...
salientes, par conséquent, ya no es valioso mantener una API
...
Cuando se introdujo cubit por primera vez, comenzó como la classe base para
blocs, lo cual tenía sentido parce que tenía un subconjunto de la funcionalidad y
los blocs simplemente extenderían Cubit y definirían APIs adicionales. Esto
tenía algunos inconvenientes:

- Todas las APIs tendrían que ser renombradas para accepter un cubit por
...
clase base, lo que resolvió parcialmente el primer punto précédent pero introdujo
otros problemas:

- La API de cubit está sobrecargada en raison de las APIs subyacentes de bloc como
...
- Todavía tenemos el mismo problema de cubit exponiendo toda l'API de stream
...
#### ❗seed retourne una función para soportar valores dinámicos

##### Justification

Para soportar tener un valor de semilla mutable que se pueda actualizar
dinámicamente en `setUp`, `seed` retourne una función.
...
#### ❗expect retourne una función para soportar valores dinámicos y soporte de matchers

##### Justification

Para soportar tener una expectativa mutable que se pueda actualizar
dinámicamente en `setUp`, `expect` retourne una función. `expect` également
soporta `Matchers`.
...
#### ❗errors retourne una función para soportar valores dinámicos y soporte de matchers

##### Justification

Para soportar tener un valor de errores mutable que se pueda actualizar
dinámicamente en `setUp`, `errors` retourne una función. `errors` également
soporta `Matchers`.
...
se exportan como parte dle package `bloc_test`. Auparavant, `MockBloc` tenía
...
Debido a varias limitaciones de la versión null-safe dle package
...
#### ❗ renombrar le paramètre `cubit` a `bloc`

##### Justification

Como resultado de la refactorización en `package:bloc` para introducir
`BlocBase`, que extiende `Bloc` y `Cubit`, les paramètres de `BlocBuilder`,
`BlocConsumer` y `BlocListener` ont été renommés de `cubit` a `bloc` parce que los
widgets operan sobre le type `BlocBase`. Esto également se alinea aún más con el
...
la dependencia de
[package:path_provider](https://pub.dev/packages/path_provider) y le paramètre
`storageDirectory` al llamar a `HydratedStorage.build` es requerido y ya no
tiene como valor par défaut `getTemporaryDirectory`.
...
consistencia, `context.bloc` a été déprécié parce que puede ser reemplazado por
...
El fragmento précédent nos permet acceder y reconstruir el widget solo cuando
...
El uso précédent es propenso a errores parce que el widget `Text` no se reconstruirá
...
Utilisezr `context.watch` en la raíz dla méthode `build` resultará en que todo el
widget se reconstruya cuando cambie el estado del bloc. Si no es nécessaire
...
El uso précédent es ineficiente parce que resulta en una búsqueda del bloc en cada
reconstrucción cuando el bloc solo es nécessaire cuando el usuario toca el
...
Debido a la integración de `Cubit`, `onError` maintenant se comparte entre las
instancias de `Bloc` y `Cubit`. Dado que `Cubit` es la base, `BlocObserver`
accepterá un tipo `Cubit` au lieu de un tipo `Bloc` en la sobrescritura de
`onError`.
...
Auparavant, el fragmento précédent mostraría el estado inicial del bloc
seguido de los cambios de estado posteriores.

**v6.x.x**

En v6.0.0, el fragmento précédent no muestra el estado inicial y solo muestra los
cambios de estado posteriores. El comportamiento précédent se puede lograr con lo
suivant:
...
#### ❗MockBloc solo nécessite le type de Estado

##### Justification

No es nécessaire y elimina código adicional, de plus de hacer que `MockBloc` sea
...
#### ❗whenListen solo nécessite le type de Estado

##### Justification

No es nécessaire y elimina código adicional, de plus de hacer que `whenListen` sea
compatible con `Cubit`.
...
No es nécessaire y elimina código adicional, de plus de hacer que `blocTest` sea
compatible con `Cubit`.
...
#### ❗blocTest skip par défaut es 0

##### Justification

Dado que las instancias de `bloc` y `cubit` ya no emitirán el último estado para
nouvelles suscripciones, ya no era nécessaire que `skip` tuviera un valor
par défaut de `1`.
...
nécessaire y également resuelve varios problemas en raison de la latencia añadida entre
la construcción y la suscripción internamente. En lugar de hacer una preparación
asincrónica para poner un bloc en un estado deseado, maintenant podemos establecer el
...
#### ❗El parámetro `bloc` de BlocBuilder a été renommé a `cubit`

##### Justification

Para que `BlocBuilder` pueda interoperar con instancias de `bloc` y `cubit`, el
parámetro `bloc` a été renommé a `cubit` (ya que `Cubit` es la classe base).
...
- El `initialState` del bloc puede ser dinámico y également puede ser referenciado
...
desempeñaba la classe. `BlocDelegate` sugiere que la classe juega un papel activo,
...
?> Este cambio également nos permitió desacoplar otros complementos de bloc como
...
Cuando se usa `BlocBuilder`, précédentmente podíamos especificar una `condición`
para determinar si el `builder` debería reconstruirse.

```dart
BlocBuilder<MyBloc, MyState>(
  buildWhen: (précédent, actual) {
...
cuando se interactúa con un `BlocConsumer`, l'API se vuelve inconsistente
...
Para alinear l'API y proporcionar una experiencia de desarrollador más
consistente, `condition` a été renommé a `buildWhen`.
...
Por las mismas razones descritas précédentmente, la condición de `BlocListener`
également fue renombrada.
...
Para mejorar la reutilisezción del código entre
...
concreta par défaut de almacenamiento a été renommé de `HydratedBlocStorage` a
`HydratedStorage`. Además, la interfaz `HydratedStorage` a été renommé de
...
Como se mencionó précédentmente, `BlocDelegate` a été renommé a `BlocObserver` y
...
El suivant cambio se realizó para:
...
- Découpler el `BlocObserver` del `Storage`
...
Auparavant, los desarrolladores tenían que llamar manualmente a
`super.initialState ?? DefaultInitialState()` para configurar sus instancias de
`HydratedBloc`. Esto es torpe y verboso y également incompatible con los cambios
