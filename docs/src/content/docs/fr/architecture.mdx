...
- Presentation
- Logic Métier (Business Logic)
- Data
  - Repository
  - Data Provider

Nous allons commencer avec la couche la plus basse (la plus éloigné de l'interfacer utilisateur) et remonter jusqu'à la couche de Présentation. 
...
Le Data Provider permet généralement d'intéragir avec des API simple afin d'effectuer des opérations [CRUD](https://en.wikipedia.org/wiki/Create,_read,_update_and_delete).
...
La fonction de la couche de Logique Métier est de répondre aux entrées provenant de la couche Présentation avec de nouveaux états. Cette couche peut dépendre d'un ou plusieurs Repository afin de récupérer les données nécessaire à la construction de l'état de l'application.
...
Le même Repository peut être injecté dans chaque bloc qui doit réagir aux nouvelles idées d'applications. Voici un `AppIdeaRankingBloc` qui creer un état pour chaque idée d'application provenant du Repository ci-dessus :
...
La plupart des flux d'applications commencent par un événement `AppStart` qui déclenche la récupération des données qui vont être présenter à l'utilisateur.
