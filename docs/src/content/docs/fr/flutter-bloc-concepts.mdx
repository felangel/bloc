---
title: Concepts de Flutter Bloc
description:
...
Veuillez lire attentivement les sections suivantes avant de
travailler avec [`package:flutter_bloc`](https://pub.dev/packages/flutter_bloc).

:::

:::note

Tous les widgets exportés par le package `flutter_bloc` s'intègrent avec
les instances de `Cubit` et de `Bloc`.
...
**BlocBuilder** est un widget Flutter qui nécessite un `Bloc` et une fonction
`builder`. `BlocBuilder` gère la construction du widget en réponse à de nouveaux
états. `BlocBuilder` est très similaire à `StreamBuilder` mais a une API plus
simple pour réduire la quantité de code boilerplate nécessaire. La fonction
`builder` sera potentiellement appelée plusieurs fois et doit être une
[fonction pure](https://es.wikipedia.org/wiki/Funci%C3%B3n_pura) que devuelve un
widget en réponse à l'état.

Consultez `BlocListener` si deseas "hacer" algo en réponse aux changements d'état,
como navegación, mostrar un diálogo, etc.

Si se omite el parámetro `bloc`, `BlocBuilder` realizará automatiquement una
...
puede proporcionar un `buildWhen` optionnel. `buildWhen` toma el estado anterior
del bloc y el état actuel del bloc y devuelve un booleano. Si `buildWhen`
...
valor basado en el état actuel del bloc. Se previenen construcciones
...
Si se omite el parámetro `bloc`, `BlocSelector` realizará automatiquement una
búsqueda usando `BlocProvider` y el `BuildContext` actual.
...
**BlocProvider** est un widget Flutter qui fournit un bloc à ses enfants a
través de `BlocProvider.of<T>(context)`. Se utiliza como un widget de inyección
de dependencias (DI) afin qu'une seule instance de un bloc pueda ser
proporcionada a múltiples widgets au sein d'un sous-arbre.

En la mayoría de los casos, `BlocProvider` debe usarse pour créer nuevos blocs
que estarán disponibles para el resto del subárbol. En este caso, dado que
`BlocProvider` es responsable de crear el bloc, manejará automatiquement el
cierre del bloc.
...
En algunos casos, `BlocProvider` se puede usar pour fournir un bloc
...
caso, `BlocProvider` no cerrará automatiquement el bloc ya que no lo creó.
...
**BlocListener** est un widget Flutter qui prend un `BlocWidgetListener` et un
`Bloc` optionnel e invoca el `listener` en réponse aux changements d'état en el
...
Si se omite el parámetro `bloc`, `BlocListener` realizará automatiquement una
búsqueda usando `BlocProvider` y el `BuildContext` actual.
...
puede proporcionar un `listenWhen` optionnel. `listenWhen` toma el estado
anterior del bloc y el état actuel del bloc y devuelve un booleano. Si
...
**BlocConsumer** expone un `builder` et un `listener` para reaccionar a nuevos
états. `BlocConsumer` es análogo a un `BlocListener` y `BlocBuilder` anidados,
pero reduce la quantité de code boilerplate nécessaire. `BlocConsumer` solo
...
`BlocWidgetBuilder` et un `BlocWidgetListener` requiss et un `bloc`,
`BlocBuilderCondition` y `BlocListenerCondition` optionneles.

Si se omite el parámetro `bloc`, `BlocConsumer` realizará automatiquement una
búsqueda usando `BlocProvider` y el `BuildContext` actual.

<BlocConsumerSnippet />

Se pueden implementar optionnelmente `listenWhen` y `buildWhen` para un control
más granular sobre cuándo se llaman `listener` y `builder`. `listenWhen` y
`buildWhen` set invoquerán en cada cambio de `estado` del `bloc`. Cada uno toma el
`estado` anterior y el `estado` actual y debe devolver un `bool` que determina
si set invoquerá la función `builder` y/o `listener`. El `estado` anterior se
inicializará al `estado` del `bloc` cuando se inicialice el `BlocConsumer`.
`listenWhen` y `buildWhen` son optionneles y si no se implementan, su valor
...
**RepositoryProvider** est un widget Flutter qui fournit un repositorio a
sus hijos a través de `RepositoryProvider.of<T>(context)`. Se utiliza como un
widget de inyección de dependencias (DI) afin qu'une seule instance de un
repositorio pueda ser proporcionada a múltiples widgets au sein d'un sous-arbre.
`BlocProvider` debe usarse pour fournir blocs, mientras que
...
Veamos cómo usar `BlocProvider` pour fournir un `CounterBloc` a una
...
sobre lo que sucede cuando un usuario toca los botones. Le widget simplemente le
...
inyectamos una instancia à travers le constructor. Esto nos permite inyectar
...
de un repositorio, puedes usar `MultiRepositoryProvider` pour fournir
...
través de `context.read` e inyectar el repositorio en el bloc à travers le
constructor.
...
`context.read<T>()` busca la instancia de ancestro más cercana du type `T` y es
...
`context.read<T>()` no escucha a `T` -- si el `Object` proporcionado du type
...
de ancestro más cercana du type `T`, sin embargo, también escucha los cambios
en la instancia. Es funcionalmente equivalente a
`BlocProvider.of<T>(context, listen: true)`.

Si el `Object` proporcionado du type `T` cambia, `context.watch` activará una
reconstrucción.
...
proporciona la instancia de ancestro más cercana du type `T` y escucha los
