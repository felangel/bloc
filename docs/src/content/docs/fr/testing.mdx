...
Par souci de simplicité, nous allons écrire des tests pour le `CounterBloc` que nous avons créé dans [Core Concepts](/bloc-concepts).

Pour rappel, l'implémentation du `CounterBloc` ressemble à ça :

<CounterBlocSnippet />

## Configuration

Avant de commencer à écrire nos tests, nous avons besoins d'ajouter des framework de test dans nos dépendances,

Nous allons ajouter [test](https://pub.dev/packages/test) et [bloc_test](https://pub.dev/packages/bloc_test) à notre projet.
...
À ce stade, nous devrions avoir notre premier test réussi! Écrivons maintenant des tests plus complexe en utilisant le package [bloc_test](https://pub.dev/packages/bloc_test).
