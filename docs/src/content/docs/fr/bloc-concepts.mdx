---
title: Concepts de Bloc
description: Un aperçu des concepts de base pour package:bloc.
sidebar:
  order: 1
---
...
travailler avec [`package:bloc`](https://pub.dev/packages/bloc).

:::

Il y a plusieurs concepts clés qui sont critiques pour comprendre comment utiliser le package
bloc.

Dans les prochaines sections, nous allons discuter chacun d'eux en détail y
et nous travaillerons également sur comment ils s'appliquent à une application de compteur.
...
pour obtenir plus d'informations sur `Streams`.

:::

Un stream est une séquence de données asynchrones.

Pour utiliser la bibliothèque bloc, il est fondamental d'avoir une compréhension de base des
`Streams` et comment ils fonctionnent.

Si vous n'êtes pas familier avec les `Streams`, pensez à un tuyau avec de l'eau
qui coule à travers. Le tuyau est le `Stream` et l'eau sont les données
asynchrones.

Nous pouvons créer un `Stream` en Dart en écrivant une fonction `async*` (generador
asíncrono).

<CountStreamSnippet />

En marquant une fonction comme `async*` nous pouvons utiliser le mot-clé `yield` y
devolver un `Stream` de données. Dans l'exemple ci-dessus, nous retournons un
`Stream` d'entiers jusqu'à le paramètre entier `max`.

Chaque fois que nous utilisons `yield` dans une fonction `async*` nous poussons cette donnée a
través del `Stream`.

Nous pouvons consommer el `Stream` anterior de varias maneras. Si nous voulions écrire
une fonction pour retourner la somme de un `Stream` de enteros, podría verse algo
así:

<SumStreamSnippet />

Al marcar la función anterior como `async` nous pouvons utiliser le mot-clé `await`
et retourner un `Future` de enteros. En este ejemplo, estamos esperando cada valor
en el stream y devolviendo la somme de todos los enteros en el stream.
...
los estados y redibujar partes de sí mismos en función del état actuel.
...
Nous pouvons créer un `CounterCubit` así:
...
inicial. En el fragmento anterior, estamos configurando l'état initial a `0`
...
acceder al état actuel del `Cubit` mediante el getter `state` y emitir un
nuevo estado sumando 1 al état actuel.
...
Luego imprimimos el état actuel del cubit, que es l'état initial (ya que no
se han emitido nuevos estados aún). A continuación, llamamos a la función
`increment` para desencadenar un changement d'état. Finalmente, imprimimos el
...
todos los cambios para un `Cubit` dado en surchargeant `onChange`.
...
`CounterIncrementPressed` entrante, podemos acceder al état actuel del bloc a
través del getter `state` y `emit(state + 1)`.

:::note

Dado que la clase `Bloc` extiende `BlocBase`, tenemos acceso al état actuel
del bloc en cualquier momento à travers le getter `state`, al igual que en
`Cubit`.
...
Luego imprimimos el état actuel del `Bloc`, que es l'état initial (ya que
no se han emitido nuevos estados aún). A continuación, agregamos el evento
`CounterIncrementPressed` para desencadenar un changement d'état. Finalmente,
...
Dado que `Bloc` extiende `BlocBase`, nous pouvons observer todos los cambios de
...
Podemos hacer esto en surchargeant `onTransition`.

El cambio de un estado a otro se llama `Transition`. Una `Transition` consiste
en el état actuel, el evento y el siguiente estado.
...
en surchargeant `onError` al igual que con `Cubit`.
...
exponer pour changer l'état. En comparación, al crear un `Bloc`, tenemos que
...
Consultez [`package:stream_transform`](https://pub.dev/packages/stream_transform)
...
Consultez [`package:bloc_concurrency`](https://pub.dev/packages/bloc_concurrency)
