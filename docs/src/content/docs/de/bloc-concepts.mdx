...
Ein `Cubit` ist eine Klasse, die `BlocBase` extended und erweitert werden kann, um jeden
...
Änderungen auszulösen, anstatt auf Funktionen. `Bloc` extended auch `BlocBase`, was bedeutet, dass es eine
...
Da die `Bloc` Klasse `BlocBase` extended, haben wir Zugriff auf den aktuellen State
...
Da `Bloc` `BlocBase` extended, können wir alle State-Änderungen für einen `Bloc`
