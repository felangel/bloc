...
:::note Please make sure to carefully read the following sections before working
with [`package:bloc`](https://pub.dev/packages/bloc). :::
...
:::note For more information about the origins of `Cubit` checkout
...
:::caution The `emit` method is protected, meaning it should only be used inside
of a `Cubit`. :::
...
:::caution Only subsequent state changes will be received when calling `listen`
on a `Cubit`. :::
...
:::note A `Change` occurs just before the state of the `Cubit` is updated. A
`Change` consists of the `currentState` and the `nextState`. :::
...
:::note All we need to do is extend `BlocObserver` and override the `onChange`
method. :::
...
:::note The internal `onChange` override is called first, which calls
`super.onChange` notifying the `onChange` in the `BlocObserver`. :::

:::tip In `BlocObserver` we have access to the `Cubit` instance in addition to
the `Change` itself. :::
...
:::note `onError` can be overridden within the `Cubit` to handle all errors for
a specific `Cubit`. :::
...
:::note Since the `Bloc` class extends `BlocBase`, we have access to the current
state of the bloc at any point in time via the `state` getter just like in
`Cubit`. :::

:::caution Blocs should never directly `emit` new states. Instead every state
change must be output in response to an incoming event within an `EventHandler`.
:::

:::caution Both blocs and cubits will ignore duplicate states. If we emit
`State nextState` where `state == nextState`, then no state change will occur.
:::
...
:::note `await Future.delayed(Duration.zero)` is added to ensure we wait for the
...
:::note `onTransition` is invoked before `onChange` and contains the event which
...
:::note `onTransition` is invoked first (local before global) followed by
`onChange`. :::
...
:::note `onEvent` is called as soon as the event is added. The local `onEvent`
is invoked before the global `onEvent` in `BlocObserver`. :::
...
:::note The local `onError` is invoked first followed by the global `onError` in
`BlocObserver`. :::

:::note `onError` and `onChange` work the exact same way for both `Bloc` and
`Cubit` instances. :::

:::caution Any unhandled exceptions that occur within an `EventHandler` are also
reported to `onError`. :::
...
:::tip See
[`package:stream_transform`](https://pub.dev/packages/stream_transform) and
