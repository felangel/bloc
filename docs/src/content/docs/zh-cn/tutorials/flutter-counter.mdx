...
:::note `Bloc` 和 `Cubit` 的 `onChange` 工作方式是相同的。:::
...
:::note我们扩展了 `MaterialApp` 因为 `CounterApp` _是_ 一个
...
:::note将 `Cubit`
的创建和消费分开并解耦非常重要，这样可以让代码更加的易于测试和重用。:::
...
:::note只有 `Text` 部件被包装在 `BlocBuilder` 里，因为它是 `CounterCubit`
