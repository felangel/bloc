...
❌ **AVOID** 在 `build` 方法當中使用`context.read` 查找狀態
...
以上的使用可能會導致UI 不會反應最新的狀態變化的錯誤，因為即使狀態改變了 `Text` widget 並不會被重新建立（rebuild），。

:::caution
使用 `BlocBuilder` 或者 `context.watch` 以確保當狀態改變時，UI 可以被正確地重新構建。
:::
