...
:::note在使用 [`package:bloc`](https://pub.dev/packages/bloc)
之前请确保已仔细阅读以下内容。:::
...
:::caution `emit` 是保护方法，意味着它只能在 `Cubit` 内部被调用。:::
...
:::note `await Future.delayed(Duration.zero)` 在这个示例里是为了避免
`subscription` 被立即取消。:::

:::caution只有在 `Cubit` 上调用 `listen` 时才会收到后续状态变化。:::
...
:::note `Change` 发生在 `Cubit` 状态更新之前。`Change` 包括 `currentState` 和
`nextState`。:::
...
:::note我们要做的仅仅是扩展 `BlocObserver` 并且重写 `onChange` 方法。:::
...
:::note内部重写的 `onChange` 先会被调用，它再调用 `super.onChange` 通知
`BlocObserver` 的 `onChange`。:::

:::tip在 `BlocObserver` 中，除了 `Change` 本身之外，我们还可以访问 `Cubit`
实例。:::
...
:::note可以在 `Cubit` 中覆盖 `onError` 来处理特定 `Cubit` 的所有错误。:::
...
:::tip `EventHandler` 可以访问添加的事件以及 `Emitter`，`Emitter`
可以对输入的事件发出零个或者多个状态。:::
...
:::note因为 `Bloc` 扩展了 `BlocBase`，所以我们可以像在 `Cubit` 中一样通过 `状态`
的 getter 随时访问 bloc 的当前状态。:::

:::caution Blocs 任何时候都不应该 `emit` 新的状态。相反，每次状态变更都必须在
`EventHandler` 里响应并输出。:::

:::caution blocs 和 cubits 都会忽略重复的状态。如果 `state == nextState`
时我们发出 `State nextState` ，不会有状态变更发生。:::
...
:::note `await Future.delayed(Duration.zero)`
是用来确保我们等待下一个事件循环周期（以便 `EventHandler` 处理这个事件）。:::
...
:::note 上面示例里的 await
Future.delayed(Duration.zero)` 仅用于防止订阅被立即取消。:::
...
:::note `onTransition` 在 `onChange` 之前被调用并且包含了触发 `currentState` 到
`nextState`的事件。:::
...
:::note `onTransition` 会被先调用（先本地再全局），然后 `onChange` 被调用。:::
...
:::note当事件被添加时，`onEvent` 会被立即调用。本地的 `onEvent` 会在
`BlocObserver` 的全局 `onEvent` 之前被调用。:::
...
:::note本地的 `onError` 方法会先被调用，然后时 `BlocObserver` 里的全局 `onError`
方法。

:::

:::note `Bloc` 和 `Cubit` 实例里的 `onError` 和 `onChange` 工作方式完全相同。:::

:::caution `EventHandler` 里未被处理的异常同样触发 `onError`。:::
...
:::tip查看
[`package:stream_transform`](https://pub.dev/packages/stream_transform) 和
[`package:rxdart`](https://pub.dev/packages/rxdart) 来了解 `Stream`
转换的细节。:::
