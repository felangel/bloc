...
:::note在开始 [`package:flutter_bloc`](https://pub.dev/packages/flutter_bloc)
请详细阅读以下文档 :::

:::note所有 `flutter_bloc` 套件导出的Widget都与 Cubit 和 Bloc 实例集成。:::
...
:::note查询更多关于 [`package:provider`](https://pub.dev/packages/provider). :::
...
:::caution使用 `BlocBuilder` 或者 `context.watch`
...
:::caution `context.watch` 只能在 `StatelessWidget` 或 `State`
类的 build 方法内部被访问。:::
...
:::caution如果在 `build` 方法的根部使用 `context.watch`
...
:::caution在 `build` 方法的根部使用 `context.select`
