...
We will start with the weather page which uses `dependency injection` in order to provide an instance of the `WeatherCubit` to the widget tree. The `WeatherPage` uses `BlocProvider` to provide an instance of the `WeatherCubit` to the subtree.
