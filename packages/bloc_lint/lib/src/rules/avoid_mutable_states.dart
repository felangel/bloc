...
/// {@template avoid_mutable_states}
/// The avoid_mutable_states lint rule.
/// {@endtemplate}
class AvoidMutableStates extends LintRule {
  /// {@macro avoid_mutable_states}
  AvoidMutableStates([Severity? severity])
    : super(name: rule, severity: severity ?? Severity.warning);

  /// The name of the lint rule.
  static const rule = 'avoid_mutable_states';
...
class _Listener extends Listener {
  ...
  /// Whether the enclosing class is a bloc state.
  bool _isStateClass = false;
  ...
  void beginClassDeclaration(
    ...
    // e.g. `sealed class CounterState {}`
    _isStateClass = name.lexeme.isStateType;
  }
  ...
  void handleClassExtends(Token? extendsKeyword, int typeCount) {
    ...
    // e.g. `final class CounterInitial extends CounterState {}`
    if (superclass.lexeme.isStateType) _isStateClass = true;
  }
  ...
  void endFields(
    ...
    if (!_isStateClass) return;
    if (kind != DeclarationKind.Class) return;

    // Static fields are not part of the state instance.
...
extension on String {
  /// Whether the type name refers to a bloc state.
  bool get isStateType => endsWith('State');
}
