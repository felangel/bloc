...
class MyThemeAppState extends State<MyThemeApp> {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<Bloc<ThemeEvent, ThemeData>, ThemeData>(
      bloc: _themeBloc,
...
void main() {
  group('BlocBuilder', () {
    ...
    testWidgets('shows latest state instead of initial state', (tester) async {
      final _themeBloc = ThemeBloc()..add(SetDarkTheme());
      await tester.pumpAndSettle();
    ...
    testWidgets('with buildWhen only rebuilds when buildWhen evaluates to true',
      ...
      final counterText1 =
          tester.widget(find.byKey(const Key('myCounterAppText'))) as Text;
      expect(counterText1.data, '0');

      final conditionalCounterText1 = tester
          .widget(find.byKey(const Key('myCounterAppTextCondition'))) as Text;
