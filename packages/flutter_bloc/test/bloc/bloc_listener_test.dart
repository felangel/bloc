        ...
        body: BlocListener<CounterBloc, int>(
          ...
          child: Column(
            children: [
              Text('count: $count'),
              RaisedButton(
                key: const Key('bloc_listener_reset_button'),
                onPressed: () {
                  setState(() => _counterBloc = CounterBloc());
                },
              ),
              RaisedButton(
                key: const Key('bloc_listener_noop_button'),
                onPressed: () {
                  setState(() => _counterBloc = _counterBloc);
                },
              ),
              RaisedButton(
                key: const Key('bloc_listener_increment_button'),
...
void main() {
  group('BlocListener', () {
    ...
    testWidgets(
      'calls listener on single state change',
      (tester) async {
        int latestState;
        ...
        await expectLater(counterBloc, emitsInOrder(expectedStates));
    ...
    testWidgets(
      'calls listener on multiple state change',
      (tester) async {
        int latestState;
        var listenerCallCount = 0;
        final counterBloc = CounterBloc();
        ...
        counterBloc..add(CounterEvent.increment)..add(CounterEvent.increment);
        await expectLater(counterBloc, emitsInOrder(expectedStates));
    ...
    testWidgets(
      ...
      final incrementFinder =
          find.byKey(const Key('bloc_listener_increment_button'));
      final resetBlocFinder =
          find.byKey(const Key('bloc_listener_reset_button'));
    ...
    testWidgets(
      ...
      final incrementFinder =
          find.byKey(const Key('bloc_listener_increment_button'));
      final noopBlocFinder = find.byKey(const Key('bloc_listener_noop_button'));
      await tester.pumpWidget(MyApp(
        onListenerCalled: (context, state) {
          listenerCallCount++;
          latestState = state;
    ...
    testWidgets(
      'infers the bloc from the context if the bloc is not provided',
      (tester) async {
        int latestPreviousState;
        int latestState;
        var listenWhenCallCount = 0;
